...
const OptionWheel = ({
  ...
  const applyTarget = useCallback(
    (value: number, snap: boolean) => {
      const cfg = cfgRef.current as { count: number; loop: boolean; items: string[] };
      let v = value;
  ...
  const sideClass = side === 'center' ? 'option-wheel--center' : side === 'right' ? 'option-wheel--right' : '';
  ...
  return (
    <div
      ...
      className={`option-wheel ${sideClass}${isDragging ? ' option-wheel--dragging' : ''}${className ? ` ${className}` : ''}`}
      style={{
